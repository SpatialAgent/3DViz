...
define(["dojo/_base/array", "dojo/_base/declare", "dojo/_base/kernel", "dojo/_base/lang", "dojo/_base/url",
  ...
  return declare([Evented], {
    ...
    customUrlConfig: {},
    appResponse: {},
    commonUrlItems: ["webscene", "group", "appid", "oauthappid"],
    ...
    _init: function () {
      ...
      this._checkSignIn().always(lang.hitch(this, function (response) {
        ...
        all({
          ...
          // get portal info - groups need this
          ...
          all({
            ...
            if ((this.appResponse && this.appResponse.access && this.appResponse.access !== "public")) { // check app access
              if (response && response.name && response.name === "identity-manager:not-authorized") {
                var licenseMessage = "<h1>" + this.i18nConfig.i18n.scene.licenseError.title + "</h1><p>" + this.i18nConfig.i18n.scene.licenseError.message + "</p>";
    ...
    queryGroupItems: function (options) {
      ...
      if (this.templateConfig.queryForGroupItems) {
        if (this.config.group) {
          // group params
          defaultParams = new PortalQueryParams({
          ...
          // place group ID
          if (params.query) {
            params.query = string.substitute(params.query, {
    ...
    queryItem: function () {
      ...
      // use webscene from id
        ...
        item.load().then(lang.hitch(this, function (data) {
          cfg.itemInfo = data;
          this.itemConfig = cfg;
          deferred.resolve(cfg);
        }));
      ...
      if (this.config.appid) {
        ...
        sceneItem.then(lang.hitch(this, function (itemData) {
          ...
          itemData.fetchData().then(lang.hitch(this, function (data) {
            ...
            // Check for app  proxies
            if (itemData && itemData.applicationProxies) {
              var layerMixins = array.map(itemData.applicationProxies, function (p) {
                return {
                  "url": p.sourceUrl,
                  "mixin": {
                    "url": p.proxyUrl
